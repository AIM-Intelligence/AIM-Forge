...
class EnhancedFlowExecutor(FlowExecutor):
    ...
    def _execute_node_isolated(
        ...
        if is_text_input:
            ...
            print(
                f"[TEXTINPUT NODE {node_id}] Final stored value: {repr(stored_value)}, Type: {type(stored_value)}"
            )
        ...
        if node_type == "result":
            ...
            if (
                ...
                print(
                    f"[RESULT NODE {node_id}] Received input_data: {str(input_data)[:200]}"
                )
                ...
                if (
                    ...
                    and input_data.get("type") == "reference"
                ):
                    ...
                    if isinstance(unwrapped, str):
                        ...
                        try:
                            json_str = json.dumps(
                                unwrapped, indent=2, ensure_ascii=False
                            )
                ...
                output_data = {
                    ...
                    "raw_value": (
                        ...
                        or input_data.get("type") != "reference"
                        else None
                    ),
                }
        ...
        try:
            ...
            print(
                f"[NODE {node_id}] Actual input before handle processing: {actual_input}"
            )
            ...
            if incoming_edges:
                if len(incoming_edges) == 1:
                    ...
                    if (
                        ...
                        print(
                            f"[CHECK] node={node_id}, sourceHandle={edge_info['sourceHandle']}, keys={list(source_output_unwrapped.keys())[:10]}"
                        )
                        if edge_info["sourceHandle"] in source_output_unwrapped:
                            value = source_output_unwrapped[edge_info["sourceHandle"]]
                            print(
                                f"[CHECK] Successfully extracted '{edge_info['sourceHandle']}': {str(value)[:100]}"
                            )
                        else:
                            print(
                                f"[CHECK] sourceHandle '{edge_info['sourceHandle']}' not found in output"
                            )
                    else:
                        print(
                            f"[CHECK] No extraction: type={type(source_output_unwrapped)}, sourceHandle={edge_info.get('sourceHandle')}"
                        )
